...
def log_dataset_info(logger: logging.Logger, data_manager: Any):
    ...
    logger.info(f"🏷️  Training class distribution:")
...
def log_experiment_end(
    ...
    logger.info(f"⏱️  Total training time: {training_time:.2f} seconds ({training_time/3600:.2f} hours)")
...
def log_class_performance(
    ...
    logger.info("🏷️  CLASS-WISE PERFORMANCE")
    logger.info("-" * 50)
...
def log_site_performance(
    ...
    logger.info(f"⚠️  Bottom {top_n} sites:")
    for site, acc in sorted_sites[-top_n:]:
        logger.info(f"   {site}: {acc:.3f}")
